...
| 分组 | 内容 |
| --- | --- |
| 外观 | 主题（八选一，默认-浅色 / 默认-深色 / 金曦 / 晓月 / 银海 / 暗影 / 红莲 已做完）；背景（无·跟随主题 / 纯色 / 图片链接 / 上传图片，图片固定铺满裁切、可调模糊·亮度）；卡片（不透明度、毛玻璃模糊） |
...
| 项 | 规则 |
| --- | --- |
| 背景 | 主题自带图（银海是 `/bg/8-evercold.webp`）就自动写进「图片链接」并把模糊/亮度改成预设值；主题不带图（只剩苍穹）就退回「无」 |

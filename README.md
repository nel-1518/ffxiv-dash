...
- **config 里只有设置**：`focusMinutes`（10-120，默认 30）/ `breakMinutes`（1-20，默认 5）/ `autoNext`（默认开）/
  `focusDoneText`（默认「专注结束，活动一下身体，休息休息眼睛。」）/ `breakDoneText`（默认「休息结束，继续冒险！」）
  —— 两句话是通知正文，**空串是合法值**＝只弹标题（上限 80 字）。
...
- **状态机**：`ready | running{phase,endsAt} | paused{phase,remainingMs} | awaiting{next}`。
  ...
  自然到点：勾了自动接续就直接跑下一段，否则停在 `0min` 等手动开始；两种都发提醒。
  「结束」是提前收尾，落点同上但**不打扰**（不发通知）。
- **时钟粒度**：父组件只订阅「是否已到点」这个**布尔快照**（到点那一秒重渲染一次），
  每秒跳动的东西全在 `Countdown` 叶子里（`useClockValue` 返回整数秒，环靠它每秒走一格；暂停 / 就绪时它是常量
  ⇒ 那两种状态**一次都不重渲染**）。**卡片上的数字只显示整分钟**（`30min`），秒级的精度由环给 ——
  数字每分钟才跳一次。换算走 `displayMinutes(seconds, maxSeconds)`，**必须把秒数夹到本段总时长**：
  读数用的时钟缓存值最多比真实时间晚 1 秒，不夹的话起点会算出 1801 秒 ⇒ 30 分钟的段显示 `31min`
  （上一版秒级读数就是这个现象的 `30:01`）。
...
- **卡面**：胶囊（**纯文字色、无底色**：专注 = 主色，休息 / 未开始 / 已暂停 = 次级色）+ 环内读数 +
  三个纯图标按钮（开始 / 暂停、结束、重置；用原生 `title` + `aria-label`，不用 antd Tooltip）。
...
- **构建产物体积**：antd + dnd-kit + marked 后主包约 1.27 MB（gzip 约 410 kB）；

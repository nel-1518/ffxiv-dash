...
- **默认-深色**：antd 深色算法（`darkAlgorithm`）的原生观感，只覆盖 `colorBgLayout`
  ...
  ⚠️ 卡片是**不透明的纯色深灰** `#1e1e1e`（`--dash-card-rgb: 30 30 30` 两端相同 + `cards.alpha: 100`）。
  以前写的是 `alpha: 8`（8% 的白压在深底上珠途同归），但那一旦用户自己设了**背景图片**
  就会整片透掉 —— 靠低 alpha 调出来的颜色只在上「底下永远是这个色」时成立。
...
- **红莲**（参考 <https://na.finalfantasyxiv.com/stormblood/>，同一套采样手法）：
  老版官网是「**深红打底 + 金**」（站点实测：导航/内容区整块 `#801111`、浮层 `#660a0a`、
  强调块 `#990f0f` + 金表头字 `#e5d473`、注释金 `#ffcc33`，正文 `#f5f5f5`）。
  ⚠️ 页面左侧那支 `#e30613` 是官网自己的品牌红（Cookie 横幅也用它），不是红莲的色；
  导航里那个 `#73bfe6` 也仍是模板公共站的蓝。
- **红莲的映射**（用户后来指定了具体值）：页面底压深一档（`#6f1111`）；
  **顶栏与弹窗是纯色 `#6F0B0B`**（页头比卡片深一档，把两层分开）；
  **卡片是「两侧暗、中间亮」的三色水平渐变** `#801111 → #970B0B → #801111`
  且 **不透明**（要的就是实心红块，不是玻璃，所以 `cards` 预设给 100 / 0）；
  标题与链接都用金（红 + 金就是站点的成套用法），卡片阴影照搬导航条的
  「内嵌上缘亮线 + 外投影」，圆角取站点的 4px 那一档。
  ⚠️ 三色水平渐变**表达不了**默认那套「两个通道值 + 不透明度」拼的 135° 两色斜渐变，
  所以在 `.dash-shell` 上重写了整条 `--dash-card-bg` ——
  必须写在 `.dash-shell` 上而不是 `[data-dash-theme]` 上（全局那条就声明在 `.dash-shell`，
  写在祖先上会被它盖回去）；三个色标都乘了 `--dash-card-alpha`，所以不透明度滑块仍然有效。
  ⚠️ **红主题的固有难题**：红色主色压在深红卡上天然对比不足（站点那支 `#990f0f` 只有 1.7:1）。
  站点自己的解法是「实心红块 + 白字」，而不是「亮红文字压红底」—— 所以这里
  `colorPrimary` 只用在 accent 竖条、primary 按钮底这类**实心图形**上
  （种子提到 `#ff8f8f` → 实测 `#dc7d7d` 才够 3:1），
  而**一切可点的文字都走金色的 `colorLink`**。
- **苍穹**（参考 <https://na.finalfantasyxiv.com/heavensward/>，同一套采样手法）：
  老版官网的「近黑底 + 冰蓝」—— 主内容区 `#141414` + 正文 `#bfbfbf`、表头深蓝黑 `#1f2633`、
  面板描边深蓝 `#324366`、**标题冰蓝白 `#e5f4ff`**、导航 hover/选中淡冰蓝 `#cceaff`、
  子导航蓝灰 `#5c7f99`、提示玫红 `#bf2659`、次级标题暖灰金 `#bfb38f`。
  映射：页面底 `#12141a`，卡片取 `#1f2633` → `#181e2a`（深蓝黑玻璃），
  标题与卡片名称都用那支冰蓝白（站点 h3/h4 就是它，在深蓝卡上约 12:1，不用再调），
  圆角取站点 6px / 10px 两档之间的 10 / 8。
  **背景带一层四周暗角**（`--dash-bg-vignette`，冷蓝黑的椭圆径向渐变）——
  那是 `global.css` 上的一个钩子（`.dash-bg::after`），主题不给这个变量就是 `none`，
  其余七套因此丝毫不受影响。
  ⚠️ 主色刻意避开了两个极端：`#cceaff` 太白（白字压上去只剩 1.4:1）、`#5c7f99` 偏灰，
  取中间那档（种子 `#9fd2ef` → 实测 `#8ab6ce`），而链接色才用那支淡冰蓝。
- **八套主题至此全部做完**（默认-浅色 / 默认-深色 / 苍穹 / 红莲 / 暗影 / 晓月 / 金曦 / 银海）。
  六套有色主题的卡片都已按最新偏好处理：**无边框**，并各自带一套实测过的对比度数据。

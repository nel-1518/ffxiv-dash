...
| 分组 | 内容 |
| --- | --- |
| 外观 | 主题（八选一，默认-浅色 / 默认-深色 / 金曦 / 晓月 / 银海 已做完）；背景（无·跟随主题 / 纯色 / 图片链接 / 上传图片，图片固定铺满裁切、可调模糊·亮度）；卡片（不透明度、毛玻璃模糊） |
...
| 项 | 规则 |
| --- | --- |
| 背景 | 主题自带图（银海是 `/bg/8-evercold.webp`）就自动写进「图片链接」并把模糊/亮度改成预设值；主题不带图（其余三套还没做）就退回「无」 |
...
所以那一项也得在浅色岛里给到）。
（**晓月是例外**：它刻意让顶栏留在暗色、只让卡片变亮，做法见下面「主题」一节。）
...
- 其余三套（苍穹 / 红莲 / 暗影）选用后会沿用基线主题，等各自配色做好。

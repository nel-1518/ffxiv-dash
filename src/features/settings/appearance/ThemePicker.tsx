...
import { applyTheme } from '../../../app/themes/appearance-sync.ts'
 ...
 * 主题：八选一。
 *
 * 选择结果落到 `core/appearance/store.ts`（主题是外观里的一个字段），
 * AppProviders 订阅它重建 antd 令牌，全树跟着换配色；不需要在这里存本地 state。
 * 名字取自主题注册表（`app/themes/`），因此这里不再自带一份 label 表。
 *
 * 顺带把外观换成该主题的一套（`applyTheme` 一次写完）：背景、卡片的不透明度与模糊
